...
export class OdmviewComponent implements OnInit {
  ...
  public odm: Document;
  public dispODM : Array<Object> = [ {  FormName: "Form Not uploaded",
  ...
  public loadODM(text: string) {
    ...
    for(var k=0; k < formselements.length ; k++) {
      ...
      var aform = { FormName: formselements[k].getAttribute('Name'), Sections: sections };
      ...
      for(var i=0;  i < itemgrouprefs.length; i++) {
        ...
        var asection = { SectionName: itemgroupdef.getAttribute('Name'), Items : items } ;
        ...
        for(var j=0 ; j < itemrefs.length; j++) {
          ...
          items.push({ Name: itemdef.getAttribute('Name'), Value: itemdef.getAttribute("DataType")});
        }
